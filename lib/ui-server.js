...
function createUiServer(sbot, config, opts) {
  ...
  function handleBlobGet(req, res) {
    ...
    // sbot.blobs.get can throw synchronously (e.g. a broken/uninitialised blob
    // store on a misconfigured host). Without this guard the throw is uncaught,
    // the request handler dies, and the connection resets — which a fronting
    // proxy then reports as an opaque 500 with no clue why. Catch it, log the
    // real error, and fail the single request cleanly instead.
    try {
      pull(
