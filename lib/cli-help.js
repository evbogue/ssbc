...
const manualEntries = {
  ...
  latest: {
    description: 'Stream the latest sequence seen for every feed in the local database (not only followed feeds).',
  ...
  'invite.use': {
    description: 'Validate one invite code and publish a follow for the provided feed.',
  ...
  ws: {
    description: 'Inspect the websocket transport that browser clients rely on.',
...
// ssb-replicate removed; ebt handles replication
const externalHelpModules = [
  { name: 'ssb-db',      prefix: '' },
...
function buildExample(name, args) {
  const parts = ['ssb-server', name]
  for (const arg of Object.keys(args || {}).sort()) {
    const type = (args[arg] && args[arg].type) ? args[arg].type : 'value'
    parts.push('--' + arg + ' <' + type + '>')
  }
  return parts.join(' ')
}
...
function addEntry(catalog, name, info) {
  const args = cloneArgs(info.args)
  catalog[name] = {
...
function loadPluginEntries() {
  ...
  for (const pluginName of fs.readdirSync(pluginsDir)) {
    ...
    let stats
    try { stats = fs.statSync(pluginPath) } catch (_) { continue }
    if (!stats.isDirectory()) continue

    const helpFile = path.join(pluginPath, 'help.js')
    if (!fs.existsSync(helpFile)) continue

    let helpModule
    try { helpModule = require(helpFile) } catch (_) { continue }

    const commands = helpModule.commands || {}
    const pluginDescription = helpModule.description
    for (const commandName of Object.keys(commands)) {
      const commandHelp = commands[commandName] || {}
      addEntry(catalog, pluginName + '.' + commandName, {
        description: commandHelp.description || pluginDescription || '',
...
function loadExternalHelpEntries() {
  const catalog = {}
  for (const { name, prefix } of externalHelpModules) {
    let helpModule
    try { helpModule = require(name + '/help') } catch (_) { continue }

    const moduleDescription = helpModule.description
    const commands = helpModule.commands || {}
    for (const commandName of Object.keys(commands)) {
      const info = commands[commandName] || {}
      const entryName = prefix ? prefix + '.' + commandName : commandName
      addEntry(catalog, entryName, {
...
function createCatalog(manifest) {
  ...
  for (const name of Object.keys(pluginEntries)) catalog[name] = pluginEntries[name]

  const externalEntries = loadExternalHelpEntries()
  for (const name of Object.keys(externalEntries)) catalog[name] = externalEntries[name]

  for (const name of Object.keys(manualEntries)) addEntry(catalog, name, manualEntries[name])
  ...
  return {
    ...
    all: () => names.map((name) => ({ name, entry: catalog[name] }))
  }
}
