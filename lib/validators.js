...
// Minimal validator registry — replaces muxrpc-validation.
// Provides get/set for named type-check functions and exposes the same
// validator map that lib/db.js and index.js rely on.
function createValidators(extra) {
  const validators = {
    object(v) { if (typeof v !== 'object' || !v) return 'type' },
    string(v) { if (typeof v !== 'string') return 'type' },
    number(v) { if (typeof v !== 'number' || isNaN(v)) return 'type' },
    boolean(v) { if (typeof v !== 'boolean') return 'type' },
    array(v)  { if (!Array.isArray(v)) return 'type' }
  }
  const api = {
    get(name) { return validators[name] },
    set(name, fn) {
      if (name && typeof name === 'object')
        for (const k in name) validators[k] = name[k].bind(api)
      else
        validators[name] = fn.bind(api)
    }
  }
  if (extra) api.set(extra)
  return validators
}
